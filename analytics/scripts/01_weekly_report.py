"""
Make module 2 ("Python") of scenario "TeamTailor-report V2 (diane)" —
code:ExecuteCode, Python
https://eu1.make.com/67084/scenarios/7106751/edit

This is a local copy of the code running inside the Make scenario, adapted to
run standalone for testing/versioning:
  - job_id / channel come from env vars instead of the Make iterator pills
    ({{1.data.job_id}}, {{1.data.channel}}), which are filled in from a
    SearchRecord on the "TT Roles Registry" data store (id 172310).
...
In Make, this module's `result` is consumed by two downstream modules: an
AddRecord into "TT Weekly Snapshots" (id 172312, key `{job_id}_{week_of}`)
and a Slack CreateMessage posting `result["slack_message"]`. See
../README.md for the full two-scenario picture.

To push a change back into Make: re-hardcode job_id/channel as
{{1.data.job_id}} / {{1.data.channel}} pills and keep API_TOKEN as-is if you
haven't yet moved the token out of the code module.
"""
import json
import os
...
candidate_names = {s: [] for s in NAME_STAGES}
total_sourced = 0
...
for app in all_applications:
    ...
    if not is_rejected and stage_name in NAME_STAGES:
        name = candidate_map.get(candidate_id, {}).get("name", "")
        if name:
            candidate_names[stage_name].append(name)
...
pct_culturafit_to_refcheck = conv_decided(rs("Reference check"), rejected["Cultural-fit interview"])
pct_refcheck_to_offered = conv_decided(rs("Offered"), rejected["Reference check"])
pct_offered_to_hired = conv_decided(rs("Hired"), rejected["Offered"])
...
if rs("Offered") > 0 and pct_offered_to_hired < 80:
    alerts.append(f"⚠️ Offer acceptance below 80%: {pct_offered_to_hired}%")
...
def pipeline_line(label, active_count, total_count, stage_key=None):
    names = candidate_names.get(stage_key, []) if stage_key else []
...
pipeline_lines = [
    ...
    pipeline_line("Reference check", active["Reference check"], rs("Reference check"), "Reference check"),
    pipeline_line("Offered", active["Offered"], rs("Offered"), "Offered"),
    pipeline_line("Hired", active["Hired"], rs("Hired"), "Hired"),
]
pipeline_block = "\n".join(pipeline_lines)
...
if rs("Cultural-fit interview") > 0:
    funnel_lines.append(f"- Cultural-fit → Reference check: {pct_culturafit_to_refcheck}% {bar(pct_culturafit_to_refcheck)}")
if rs("Reference check") > 0:
    funnel_lines.append(f"- Reference check → Offered: {pct_refcheck_to_offered}% {bar(pct_refcheck_to_offered)}")
if rs("Offered") > 0:
    funnel_lines.append(f"- Offered → Hired: {pct_offered_to_hired}% {bar(pct_offered_to_hired)}")
...
result = {
    ...
    "hired_candidates": ", ".join(candidate_names["Hired"]),
}
