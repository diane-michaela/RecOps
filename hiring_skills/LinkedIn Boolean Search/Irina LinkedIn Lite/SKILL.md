---
name: LinkedIn Boolean Search — Lite
description: Acts as a sourcing advisor first, Boolean-string builder second, for a LinkedIn Recruiter Lite account (Diane's own tier). Given a JD, intake-meeting notes, or a sourcing brief, the default output is a short advisory pass — contradictions found between sources (each requirement tagged reinforced/downgraded/untouched, never silently bucketed), which asks LinkedIn structurally can't verify (paired with a partial-proxy research angle like Post Search or a feeder-company directory, not just "ask in interview"), and open questions that need Diane's call, including whether "remote" actually means worldwide or region-restricted. Also proactively surfaces further research angles (Post Search, Groups, feeder-company directories, GitHub cross-reference) tied to whichever traits turned out to be unfilterable, rather than waiting to be asked. Boolean strings are built only when explicitly requested afterward, mining every named tool/technique from the source text first, using Lite's hidden operators (headline:, summary:, skills:, yoe:...), and never LinkedIn's unreliable calculated filters (seniority, company size/type, function) as a primary cut. Named after Irina Shamaeva, co-author of the source book. Use whenever Diane pastes a JD, intake notes, or a sourcing brief and wants sourcing input on her Recruiter Lite account — including "review this JD", "what should I watch out for", "sanity-check this brief", as well as "build/refine a boolean search" once the advisory pass is done. If the account in play is full LinkedIn Recruiter instead, use the "LinkedIn Boolean Search — Recruiter" skill.
---

# LinkedIn Boolean Search — Lite — Sourcing Advisor
...
`wiki/insights/`) for the complete source context. Tuned for **LinkedIn Recruiter Lite**.
...
6. **Indirect-search opportunities**, if the direct keyword approach looks like it'll under-cover
   ...
   cross-referencing instead of or alongside a LinkedIn-only search.
7. **Diversity considerations**, only if asked for — include Greek-letter/affinity-org
   OR-strings (sororities, historically Black fraternities, Latino Greek orgs, Asian-American
   fraternities, LGBTQ+ campus orgs) alongside the school/association angles in the cheatsheet.
8. **Open questions for Diane** — anything genuinely ambiguous, phrased as a direct question, not
   silently defaulted and mentioned only in hindsight. **A JD saying "remote" is never itself an
   answer to the location question** — always ask whether that means genuinely worldwide or
...
pullable, or a GitHub cross-reference if the skill is self-titled/hard to keyword-search. This
...
### Layer in hidden Recruiter Lite operators

Undocumented, confirmed working on **Recruiter Lite**, entered in the **Job Title** or
**Company** field, not Keywords. Only apply the ones flagged as relevant in Phase A step 4; full
...
Recruiter Lite has no meaningful Boolean length cap (that's a Premium/Basic-only problem) — never
...
Only if Phase A flagged it or Diane asks directly. Indirect keyword heuristics only, several
combined for partial coverage, never a certainty about any individual profile — technique
reference in `references/operators-cheatsheet.md`.
...
location). **Recruiter Lite has no native bulk-CSV import** (full-Recruiter-only) — but check the
free option before reaching for a paid tool: **if a colleague has a full Recruiter seat, ask them
to add you as a Hiring Manager on one of their Projects** — that access includes bulk email
upload, free, without either side needing a second paid seat. Only if that's not available,
bulk-upload identifiers into a third-party contact-enrichment tool instead (SalesQL / ContactOut /
SeekOut / Clearbit), or for small batches, search the exact first+last name manually (reveals
full out-of-network profiles on any tier). Reuse existing workspace tooling
(`github_extraction/lisp.py`, the PhantomBuster pipeline) rather than rebuilding it.
