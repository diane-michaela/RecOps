...
| Engineering / technical — **library-to-contributor variant** (reported, not yet live-tested — see README) | Instead of anchoring on a generic tech term, resolve the role to the **specific library/framework** its stack actually depends on first, then target that library's own repo: `site:github.com "<library-name>"` narrowed to contributors/forkers, or a project-level search on repos that depend on it. Sharper than a generic tech-term anchor for the same reason the project-level technique above is — it finds people who've demonstrably touched the exact tool, not just people who mention it in a bio. Try this before falling back to the generic anchor-trio row above for a role with a clearly named, distinctive dependency (not a ubiquitous one like `react` or `python`) |
